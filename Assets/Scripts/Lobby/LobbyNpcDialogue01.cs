...
public class LobbyNpcDialogue01 : MonoBehaviour
{
    [Header("Visual anim")]
    ...
    void Update()
    {
        if (playerInRange && !OpenDialogueManager.GetInstance().dialogueIsPlaying){
            visualCue.SetActive(true);
            if (InputManager.GetInstance().GetInteractPressed()){
                Debug.Log("robotTest");
                OpenDialogueManager.GetInstance().EnterDialogueMode(inkJSON);
