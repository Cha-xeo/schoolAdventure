...
public class TutoFindGame : MonoBehaviour
{
    ...
    void Start()
    {
        if (PlayerPrefs.HasKey("tutoFindGame") && PlayerPrefs.GetInt("tutoFindGame") == 1) {
    ...
    void ChangeAnimation()
    {
        ...
        if (animtut.GetInteger("Change") > 3) {
            //gameObject.SetActive(false);
            if (!PlayerPrefs.HasKey("tutoFindGame")) {
                PlayerPrefs.SetInt("tutoFindGame", 1);
            }
        }   
    }
    void Update()
