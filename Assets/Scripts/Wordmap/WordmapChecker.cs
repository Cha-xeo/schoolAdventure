...
public class WordmapChecker : MonoBehaviour
{
    ...
    {
        WordmapGameEvents.OnCheckSquare += SquareSelected;
        WordmapGameEvents.OnClearSelection += ClearSelection;
        WordmapGameEvents.OnLoadNextLevel += LoadNextGameLevel;
    }
    ...
    {
        SceneManager.LoadScene("WordmapGame");
    }
    ...
    void Update()
    {
        ...
        {
            Debug.DrawRay(_rayUp.origin, _rayUp.direction * 8);
            Debug.DrawRay(_rayDown.origin, _rayDown.direction * 8);
            Debug.DrawRay(_rayLeft.origin, _rayLeft.direction * 8);
            Debug.DrawRay(_rayRight.origin, _rayRight.direction * 8);
            Debug.DrawRay(_rayDiagonalLeftUp.origin, _rayDiagonalLeftUp.direction * 8);
            Debug.DrawRay(_rayDiagonalLeftDown.origin, _rayDiagonalLeftDown.direction * 8);
            Debug.DrawRay(_rayDiagonalRightUp.origin, _rayDiagonalRightUp.direction * 8);
            Debug.DrawRay(_rayDiagonalRightDown.origin, _rayDiagonalRightDown.direction * 8);
        }
    }
    ...
    private void SquareSelected(string letter, Vector3 squarePosition, int squareIndex)
    {
        if (_assignedPoints == 0) {
            ...
            WordmapGameEvents.SelectSquareMethod(squarePosition);
            _word += letter;
            CheckWord();
        } else {
    ...
    private void CheckWord()
    {
        foreach (var searchingWord in currentGameData.selectedBoardData.SearchWords) {
            if (_word == searchingWord.Word && searchingWord.Found == false) {
                ...
                WordmapGameEvents.CorrectWordMethod(_word, _correctSquareList);
                _completedWords++;
                _word = string.Empty;
                _correctSquareList.Clear();
                CheckBoardCompleted();
    ...
    {
        var hits = Physics2D.RaycastAll(currentRay.origin, currentRay.direction);
    ...
    private Ray SelectRay(Vector2 firstPosition, Vector2 secondPosition)
    {
        ...
        if (System.Math.Abs(direction.x) < tolerance && System.Math.Abs(direction.y - 1f) < tolerance)
            return _rayUp;
        if (System.Math.Abs(direction.x) < tolerance && System.Math.Abs(direction.y - (-1f)) < tolerance)
            return _rayDown;
        if (System.Math.Abs(direction.x - (-1f)) < tolerance && System.Math.Abs(direction.y) < tolerance)
            return _rayLeft;
        if (System.Math.Abs(direction.x - 1f) < tolerance && System.Math.Abs(direction.y) < tolerance)
            return _rayRight;
    ...
    private void CheckBoardCompleted()
    {
        ...
        if (currentGameData.selectedBoardData.SearchWords.Count == _completedWords) {
            ...
            var currentBoardIndex = WordmapDataSaver.ReadCategoryCurrentIndexValue(categoryName);
            ...
            for (int index = 0; index < gameLevelData.data.Count; index++) {
                if (readNextLevelName) {
                    nextBoardIndex = WordmapDataSaver.ReadCategoryCurrentIndexValue(gameLevelData.data[index].categoryName);
            ...
            WordmapDataSaver.SaveCategoryData(categoryName, currentBoardIndex);
            ...
            if (currentBoardIndex >= currentLevelSize) {
                ...
                WordmapGameEvents.BoardCompletedMethod();
            }
            if (loadNextCategory)
                WordmapGameEvents.UnlockNextCategoryMethod();
            WordmapGameEvents.BoardCompletedMethod();
        }
    }
}
