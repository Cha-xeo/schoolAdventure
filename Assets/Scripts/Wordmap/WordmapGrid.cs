...
public class WordmapGrid : MonoBehaviour
{
    public WordmapGameData currentGameData;
    public GameObject gridSquarePrefab;
    public WordmapAlphaData alphabetData;
        ...
        if (currentGameData != null) {
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board) {
                foreach(var squareLetter in squares.Row) {
                    ...
                    var correctLetterData = alphabetData.AlphabetHighlight.Find(data => data.letter == squareLetter);
                    
                    if (normalLetterData.image == null || selectedLetterData.image == null) {
                        Debug.Log("Couldn't find letter : " + squareLetter);
                    } else {
                        _squareList.Add(Instantiate(gridSquarePrefab));
                        _squareList[_squareList.Count - 1].GetComponent<WordmapGridSquare>().SetSprite(normalLetterData, selectedLetterData, correctLetterData);
                        ...
                        _squareList[_squareList.Count - 1].transform.localScale = squareScale * 2; // x2 square size
                        _squareList[_squareList.Count - 1].GetComponent<WordmapGridSquare>().SetIndex(_squareList.Count - 1);
                    }
                }
            }
        }
    ...
    {
        var finalScale = defaultScale * 2; // Avoid tiny squares
